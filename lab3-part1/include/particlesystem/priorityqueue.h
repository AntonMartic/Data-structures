 ...
 */
template <class Comparable>
Comparable PriorityQueue<Comparable>::findMin() { // min heap, smallest on first
...
bool PriorityQueue<Comparable>::isMinHeap() const {
    ...
    */
    ...
    for (int i = 1; i <= size / 2; ++i) { // size/2 because last level (leaves) wont have any childs
        int left = 2 * i;
        int right = 2 * i + 1;

        if (left <= size && pq[i] > pq[left])
            return false;
        if (right <= size && pq[i] > pq[right])
            return false;
    }
...
void PriorityQueue<Comparable>::percolateDown(int index) {
    int size = pq.size() - 1;
    while (2 * index <= size) { // as long as we have a left child
        int child = 2 * index;
        if (child + 1 <= size && pq[child + 1] < pq[child])
            // if right child exist and the value is smaller than left child
            child++;
